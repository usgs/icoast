  ...
  $logOutput = "";
    ...
    do {
      ...
      foreach ($imagesToResize as $image) {
        ...
        if (!isset($image['dataset_id']) && !isset($image['filename']) && !isset($image['full_url'])
            && !isset($image['$image_id']) && !is_numeric($image['dataset_id']) &&
        ...
        if (!imagecopyresampled($resizedImage, $originalImage, 0, 0, 0, 0, RESIZED_WIDTH,
        ...
        // Save the image.
        if (imagejpeg($resizedImage,
            "displayImages/{$image['dataset_id']}/{$image['filename']}",
                JPEG_QUALITY)) {
