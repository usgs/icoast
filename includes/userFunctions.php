 ...
 */
function timeDifference($startDateTime,
                        $endDateTime,
                        $verbose = true)
{
 ...
 * @param int|type $postCollectionId Optional. Default = 0. iCoast DB row id of the collection to be
 *                                   used as the post image pool.
 * @param int|type $preCollectionId  Optional. Default = 0. iCoast DB row id of the collection to be
 *                                   used as the pre image pool.
 * @param int|type $userId           Optional. Default = 0. iCoast DB row id of the user.
 *
 * @param bool     $imageGroupSearch
 *
 * @return bool|int If sucessful returns a random image id <b>OR</b><br>
 * On failure returns FALSE.
 */
function random_post_image_id_generator($DBH,
                                        $projectId,
                                        $isFiltered,
                                        $postCollectionId = 0,
                                        $preCollectionId = 0,
                                        $userId = 0,
                                        $imageGroupSearch = false)
{
//       print "<p><b>In random_post_image_id_generator function.</b><br>Arguments:<br>ProjectId = $projectId<br>
//        Filtered = $isFiltered<br>Post Collection = $postCollectionId<br>Pre Collection = $preCollectionId<br>
//UserId = $userId<br>ImageGroupSearch = $imageGroupSearch</p>";
    if (!is_null($projectId) && is_Numeric($projectId) &&
        !is_null($isFiltered) && is_bool($isFiltered) &&
        is_Numeric($postCollectionId) &&
        is_Numeric($preCollectionId) &&
        is_Numeric($userId) &&
        (!$imageGroupSearch || ($imageGroupSearch && $userId > 0))
    )
    {
        $projectData =
        ...
        if ($imageGroupSearch)
        {
//            print "Building Image Group Pool<br>";
            $userGroups =
            ...
            if ($userGroups)
            {
//                print "User is in a user group<br>";
                $imageGroups =
                ...
                if ($imageGroups)
                {
//                    print "User is in a image group<br>";
                    $imageIdPool =
                ...
                {
//                    print "User is NOT in a image group<br>";
//                    print "Building standard image pool.<br>";
                    $imageIdPool =
                        retrieve_image_id_pool($projectData['post_collection_id'],
                                               false,
                                               true);
                }
            }
            ...
            {
//                print "User is NOT in a user group<br>";
//                print "Building standard image pool.<br>";
                $imageIdPool =
                    retrieve_image_id_pool($projectData['post_collection_id'],
                                           false,
...
//            print "Building standard image pool.<br>";
        ...
        asort($imageIdPool);
//        print count($imageIdPool) . ' images in Image Pool<pre>';
        if (count($imageIdPool) < 1000)
        {
...
//            print '</pre>';
        ...
        if (is_array($imageIdPool) && count($imageIdPool) > 0 && !$isFiltered)
        {
            ...
            $randomId =
                $imageIdPool[mt_rand(0,
                                     $imagesCount - 1)];
//             print "RETURNING: $randomId Unifltered Random Image Id<br>";
            return $randomId;
        }
        elseif (is_array($imageIdPool) && count($imageIdPool) > 0 && $isFiltered)
        {
            if ($userId != 0)
            {
                $userAnnotations =
                    all_user_annotated_images($userId,
                                              $projectId);
                asort($userAnnotations);
//                print 'User Annotations<pre>';
//                print_r($userAnnotations);
//                print '</pre>';
                for ($i = 0; $i < count($imageIdPool); $i++)
                {
                    if (in_array($imageIdPool[$i],
                                 $userAnnotations))
                    {
                        array_splice($imageIdPool,
                                     $i,
                                     1);
                        $i--;
                    }
...
//            print count($imageIdPool) . ' unclassified images in Image Pool<pre>';
            if (count($imageIdPool) < 1000)
            {
...
//                print '</pre>';
            ...
            while (!empty($imageIdPool))
            {
                ...
                $randomIndex =
                    mt_rand(0,
                            $imagesCount - 1);
                if (array_key_exists($randomIndex,
                                     $imageIdPool))
                {
                    $randomId = $imageIdPool[$randomIndex];
//                    print 'Random ID ' . $randomId . ' selected for filtering.<br>';
                    array_splice($imageIdPool,
                                 $randomIndex,
                                 1);
                    $imageMatchData =
                    ...
                    {
//                        print "FILTERING: Failed on is-Enabled<br>";
                    }
                }
//                print '<br>';
            }
        ...
        elseif (is_array($imageIdPool) && count($imageIdPool) == 0)
        {
            return 'poolEmpty';
        }
    }
 ...
 * On failure returns FALSE.
 */
function post_image_pool_generator($DBH,
                                   $projectId,
                                   $isFiltered,
                                   $postCollectionId = 0,
                                   $preCollectionId = 0,
 ...
 */
function all_user_annotated_images($userId,
                                   $projectId = 0)
{
//    print "In all_user_annotated_images<br>";
    if (empty($userId) || !is_numeric($userId) || !is_numeric($projectId))
    {
        return false;
    }
//    print 'Valid inputs<br>';
...
//        print 'Has result<br>';
        ...
        while ($imageId = $annotatedImagesResult->fetch(PDO::FETCH_ASSOC))
        {
//            print $imageId . '<br>';
 ...
 */
function has_user_annotated_image($DBH,
                                  $postImageId,
                                  $userId,
                                  $projectId = 0)
{
 ...
 */
function find_assigned_image_groups($DBH,
                                    $userGroups,
                                    $idOnly = false)
{
...
//      print "</pre></p>";
    ...
    {
//        print '<br>Usergroup in loop:<pre>';
//        print_r($userGroup);
//        print '</pre>';
        if (is_array($userGroup) && isset($userGroup['user_group_id']))
        {
    ...
    $imageGroupIdQuery = "SELECT image_group_id FROM user_group_image_assignments WHERE user_group_id IN "
                         . "($whereString)";
    ...
    if (count($imageGroups) > 0)
    {
        ...
        else
        {
            ...
            $STH =
                run_prepared_query($DBH,
                                   $imageGroupMetadataQuery,
                                   $imageGroupMetadataParams);
 ...
 * table in iCoast DB <b>OR</b><br> On failure or no image groups found retuns FALSE.
 */
function find_user_group_membership($DBH,
                                    $userId,
                                    $projectId = 0,
                                    $idOnly = false)
{
    ...
    if (!is_null($userId) && is_numeric($userId) && is_numeric($projectId) && is_Bool($idOnly))
    {
        ...
        if (count($userGroupIds) > 0)
        {
            ...
            $STH =
                run_prepared_query($DBH,
                                   $userGroupDetailsQuery,
                                   $userGroupDetailsParams);
 ...
 *                         be checked to ensure they contain a valid match in the matches table of the iCoast DB
 *                         (should
 ...
 *                         that will be returned. Value of 1 willr eturn a total of 3 images. 1 each side and the
 *                         supplied image in the middle.
 ...
 * <b>OR</b><br> On failure returns FALSE.
 */
function find_adjacent_images($DBH,
                              $photoId,
                              $projectId = null,
                              $userId = null,
                              $range = 1,
...
EOL;
    ...
    $imagesInCollectionResult =
        run_prepared_query($DBH,
                           $imagesInCollectionQuery,
                           $imagesInCollectionParams);
...
EOL;
    foreach ($DBH->query($adjacentImageQuery,
                         PDO::FETCH_ASSOC) as $adjacentImage)
    {
            ...
            if (isset($adjacentImageMetadata[$tempPositionCounter]))
            {
                ...
                if ($projectMetadata)
                {
                    $match =
                        retrieve_image_match_data($DBH,
                                                  ...
                                                  $adjacentImageMetadata[$tempPositionCounter]['image_id']);
                ...
                if ($hasUserAssignedImages)
                {
                    if ($adjacentImageMetadata[$tempPositionCounter]['is_globally_disabled'] == 0 &&
                        in_array($adjacentImageMetadata[$tempPositionCounter]['image_id'],
                                 $userAssignedImageIdPool)
                    )
                    {
