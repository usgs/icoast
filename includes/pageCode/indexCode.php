...
$tagFocusedProjectButtonHTML = '<input type="button" class="clickableButton" id="tagFocusedProject" value="Tag Photos From The ' . $projectInFocus['name'] . ' Project"><br>';
...
if ($userData) {
    ...
    switch ($userType) {
        ...
        <p>Check out the current iCoast project showing aerial photographs taken before and after <a href="start.php?requestedProjectId=' . $projectInFocus['project_id'] . '">' . $projectInFocus['name'] . '</a>.</p>';
            break;
        case 'existing':
            ...
            $annotatedFocusedProjectCheckResult = run_prepared_query($DBH, $annotatedFocusedProjectCheckQuery, $annotatedFocusedProjectCheckParams);
            ...
            if ($annotatedFocusedProjectCheckResult->fetchColumn() == 0) {
                $focusedProjectMessageHTML = '<p>Check out the current iCoast project showing aerial photographs taken before and after <a href="start.php?requestedProjectId=' . $projectInFocus['project_id'] . '">' . $projectInFocus['name'] . '</a>.</p>';
            }
...
EOL;
                ...
                foreach ($DBH->query($classifiedPhotoQuery, PDO::FETCH_ASSOC) as $classifiedImage) {
                    if ($numberOfImagesClassifiedByUser == 0 || !in_array($classifiedImage['image_id'], $usersClassifications)) {
...
EOL;
    } else if ($redirectError == "canceled") {
        $variableHomeContent = <<<EOL
                    ...
                    <p>Click the button below to <span class="italic">Login</span> or <span class="italic">Register</span>
                        using Google.</p>
EOL;
    } else if ($redirectError == "invalidEmail") {
        $variableHomeContent = <<<EOL
                    ...
                    <p>Click the button below to <span class="italic">Login</span> or <span class="italic">Register</span>
                        using Google.</p>
EOL;
    } else if ($redirectError == "databaseConnection") {
        $variableHomeContent = <<<EOL
...
EOL;
        ...
        $variableHomeContent = <<<EOL
                    ...
                    <p>Click the button below to <span class="italic">Login</span> or <span class="italic">Register</span>
                        using Google.</p>
EOL;
    } else if ($redirectError == "CSRFToken") {
        $variableHomeContent = <<<EOL
                    ...
                    <p>Click the button below to <span class="italic">Login</span> or <span class="italic">Register</span>
                        using Google.</p>
EOL;
    } else if ($redirectError == "IDTokenExchange") {
        $variableHomeContent = <<<EOL
