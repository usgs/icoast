...
{
    if ($projectMetadata['creator'] != $userId ||
...
$projectPropertyToUpdate =
    filter_input(INPUT_POST,
                 'projectPropertyToUpdate');
$projectEditSubAction =
    filter_input(INPUT_POST,
                 'projectEditSubAction');
    ...
    switch ($projectPropertyToUpdate)
    {
        case 'tasks':
            if (isset($projectEditSubAction) &&
                ...
                $projectEditSubAction !== 'deleteExistingTask'
            )
            {
        ...
        case 'groups':
            if (isset($projectEditSubAction) &&
                ...
                $projectEditSubAction !== 'deleteExistingGroup'
            )
            {
                $projectEditSubAction = false;
            }
        ...
        case 'tags':
            if (isset($projectEditSubAction) &&
                ...
                $projectEditSubAction !== 'deleteExistingTag'
            )
            {
                $projectEditSubAction = false;
            }
...
if ($referingPage == 'reviewProject' && $importStatus == 50)
{
    ...
    $updateTasksCompleteResult =
        run_prepared_query($DBH,
                           $updateTasksCompleteQuery,
                           $updateTasksCompleteParams);
    ...
    )
    {
        ...
        if ($continueProcessing)
        {
            ...
            while ($groupToClone = $groupsToCloneResult->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $groupMap[$groupToClone['tag_group_id']] =
                    run_prepared_query($DBH,
                                       $insertNewGroupQuery,
                                       $insertNewGroupParams,
                                       true);
                if (empty($groupMap[$groupToClone['tag_group_id']]))
                {
                    remove_rows($groupMap,
                                'tag_group_metadata');
                    remove_rows($taskMap,
                                'task_metadata');
                    $continueProcessing = false;
        ...
        if ($continueProcessing)
        {
            ...
            foreach ($taskContentsArray as $taskContentsRow)
            {
                ...
                $newTaskContentsId =
                    run_prepared_query($DBH,
                                       $insertTaskContentRowQuery,
                                       $insertTaskContentRowParams,
                                       true);
                if (empty($newTaskContentsId))
                {
                    remove_rows($newTaskContentsIdArray,
                                'task_contents');
                    remove_rows($groupMap,
                                'tag_group_metadata');
                    remove_rows($taskMap,
        ...
        if ($continueProcessing)
        {
            ...
            $findGroupGroupsResults =
                run_prepared_query($DBH,
                                   $findGroupGroupsQuery,
                                   $findGroupGroupsParams);
            ...
            $tagGroupContentsToReplicateResult =
                run_prepared_query($DBH,
                                   $tagGroupContentsToReplicateQuery);
            ...
            {
                if (in_array($tagGroupContentsRow['tag_group_id'],
                             $groupGroupIds))
                {
                ...
                {
                    remove_rows($newTagGroupContentsIdArray,
                                'tag_group_contents');
                    remove_rows($tagMap,
                                'tags');
                    remove_rows($newTaskContentsIdArray,
        ...
        if ($continueProcessing)
        {
            ...
            {
                if (!mkdir("images/projects/{$projectMetadata['project_id']}/tooltips",
                           0775,
                           true)
                )
                {
                ...
                chmod("images/projects/{$projectMetadata['project_id']}",
                      775);
                chmod("images/projects/{$projectMetadata['project_id']}/tooltips",
                      775);
            }
            else
            {
            ...
            { // iterate files
                if (is_file($file))
                {
                    ...
                    $copyResult =
                        copy($file,
                             "images/projects/{$projectMetadata['project_id']}/tooltips/{$filename}"); // copy file
                    if (!$copyResult)
                    {
                        $abort = true;
                        break;
        ...
        else
        {
            $projectUpdateErrorHTML = <<<EOL
                <h3>Clone Failed</h3>
                <p class="error">An error occurred during the clone. No changes have been made.</p>
...
if ($editSubmittedFlag && $projectPropertyToUpdate)
{
    ...
    switch ($projectPropertyToUpdate)
    {
        ...
        case 'tasks':
            ...
            if ($projectEditSubAction == 'deleteExistingTask')
            {
            ...
            else
            {
                ...
                if (!$invalidRequiredField)
                {
                    ...
                    ';
                        ...
                        $taskAlreadyPresentResult =
                            run_prepared_query($DBH,
                                               $taskAlreadyPresentQuery,
                                               $taskAlreadyPresentParams);
                ...
                if ($invalidRequiredField)
                {
                    ...
                    {
                        $failedSubmissionHTML .= 'The following required fields were either missing user input or contained invalid data:' .
                                                 $failedFields;
                    }
                    else
                    {
...
EOL;
                ...
                if (!$databaseUpdateFailure && !$invalidRequiredField)
                {
                    ...
                    $dbTaskMetadata['description'] =
                        restoreSafeHTMLTags(htmlspecialchars($dbTaskMetadata['description']));
...
EOL;
        ...
        case 'groups':
            ...
            if ($projectEditSubAction == 'deleteExistingGroup')
            {
                $invalidDeletionRequest = false;
...
MySQL;
                        ...
                        for ($i = 0; $i < $numberOfChildren; $i++)
                        {
                            ...
                            if ($parentGroupChildren[$i]['tag_id'] == $groupId) {
                                ...
                                continue;
                            } elseif ($parentGroupChildren[$i]['order_in_group'] != $sequentialOrderInProjectNumber)
                            {
                                $parentGroupChildren[$i]['order_in_group'] = $sequentialOrderInProjectNumber;
                            }
                            else
                            {
                                $parentGroupChildren[$i] = null;
...
EOL;
                ...
                else
                {
                    $projectUpdateErrorHTML = <<<EOL
                        ...
                        <div class="updateFormSubmissionControls"><hr>
                        <input type="button" class="clickableButton" id="editGroupAgain" title="This will 
                            return you to the current group for further changes and updates" value="Edit This Group  Again">
                            $taskEditingButtonHTML
                            <input type="button" class="clickableButton" id="returnToActionSelection"
                                title="This will return you to the Question Builder menu
                                for you to choose another item to create or edit."
...
EOL;
            ...
            else
            {
                ...
                {
                    if ($projectEditSubAction == 'updateExistingGroup' && empty($groupId))
                    {
                        $invalidRequiredField[] = 'groupId';
                    }
                    else
                    {
                        if ($projectEditSubAction == 'updateExistingGroup')
                        {
                            $groupMetadata =
                                retrieve_entity_metadata($DBH,
                                                         $groupId,
                ...
                )
                {
                    ...
                    if (!$invalidRequiredField)
                    {
                        ...
                        if (($newGroupParentType != $oldGroupParentType) || ($newGroupParentId != $oldGroupParentId))
                        {
                            ...
                            $newContainerNumberOfSiblings = count($newParentContainerOrder);

                            if ($newContinerNumberOfSiblings > 0)
                            {
                            ...
                            $oldContainerNumberOfSiblings = count($oldParentContainerOrder);
                            ...
                            if ($newContainerNumberOfSiblings > 0)
                            {
                                if ($newGroupParentType == 'task')
                                {
                                ...
                                print $newParentContainerUpdateQuery;
                                $newParentContainerOrderResult = $DBH->query($newParentContainerUpdateQuery);
                            }
                            if ($newParentContainerOrderResult || $newContainerNumberOfSiblings == 0)
                            {
                                //////////////////////////////////////////////////////////////////////////////
                                ...
                                if ($oldparentContainerOrderResult)
                                {
                                    if ($oldGroupParentType == 'task' && $newGroupParentType == 'task')
                                    {
                                        ...
                                        if (!$groupUpdateResults ||
                                            ($groupUpdateResults && $groupUpdateResults->rowCount() != 1)
                                        )
                                        {
                                    ...
                                    {
                                        if ($oldGroupParentType == 'group' && $newGroupParentType == 'group')
                                        {
                                        ...
                                        {
                                            if ($oldGroupParentType == 'task' && $newGroupParentType == 'group')
                                            {
                                                ...
                                                if ($groupInsertResult->rowCount() == 1)
                                                {
                                                    ...
                                                    if (!$groupDeleteResult ||
                                                        ($groupDeleteResult && $groupDeleteResult->rowCount() != 1)
                                                    )
                                                    {
                                            ...
                                            {
                                                if ($oldGroupParentType == 'group' && $newGroupParentType == 'task')
                                                {
                            ...
                            if ($groupOrderChanged)
                            {
                                foreach ($parentContainerOrder as &$individualGroup)
                                {
                                    ...
                                    elseif ($groupOrderChanged == 'down' &&
                                            $individualGroup ['child_id'] != $updateGroupId
                                    )
                                    {
                                        if ($individualGroup ['order_number'] < $oldGroupOrderInParent &&
                                            $individualGroup ['order_number'] >= $newGroupOrderInParent
                ...
                else
                {
                    if ($projectEditSubAction == 'createNewGroup')
                    {
                        ...
                        $groupAlreadyPresentResult =
                            run_prepared_query($DBH,
                                               $groupAlreadyPresentQuery,
                                               $groupAlreadyPresentParams);
...
EOL;
                    ...
                    else
                    {
                        ...
                        {
                            print '<pre>';
                            print_r($databaseUpdateFailure);
                            print '</pre>';
                            $projectUpdateErrorHTML = <<<HTML
                                <h3>Update Failed</h3>
                                <p>An unknown error occurred during the database update. No changes have been made.
                                    If this problem persists please contact an iCoast System Administrator. </p>
                                <div class="updateFormSubmissionControls"><hr>
                                ...
                                $taskEditingButtonHTML
                                <input 
                                    type="button" class="clickableButton" id="returnToActionSelection"
                                    title="This will return you to the Question Builder menu to choose another 
                                        item to create or edit."
                                    value="Return To Question Builder Menu"
                                >
                                </div>
...
EOL;
        ...
        case 'tags':
            ...
            if ($projectEditSubAction == 'deleteExistingTag') {
...
EOL;
            ...
            } else
            {
                ...
                {
                    if ($projectEditSubAction == 'updateExistingTag' && empty($tagId))
                    {
                        $invalidRequiredField[] = 'tagId';
                    }
                    else
                    {
                        if ($projectEditSubAction == 'updateExistingTag')
                        {
                            $tagMetadata =
                                retrieve_entity_metadata($DBH,
                                                         $tagId,
                ...
                if (empty($newNoTooltipImageStatus) && $newTagToolTipImage)
                {
                    ...
                    {
                        $dotLocation =
                            stripos($newTagToolTipImage,
                                    '.');
                        if ($dotLocation)
                        {
                            $fileExtension =
                                substr($newTagToolTipImage,
                                       $dotLocation + 1);
                ...
                )
                {
                    ...
                    if (!$invalidRequiredField)
                    {
                        ...
                        if (!$newNoTooltipImageStatus &&
                            !empty($newTagToolTipImage) &&
                            $oldTagTooltipImage != $newTagToolTipImage
                        )
                        {
                            ...
                            while (true)
                            {
                                ...
                                $newImage =
                                    imagecreatetruecolor($newTagToolTipImageWidth,
                                                         $newTagToolTipImageHeight);
                                // Copy the original to the new display image canvas resizing as it copies.
                                if (!imagecopyresampled($newImage,
                                                        $originalImage,
                                                        0,
                                                        0,
                                                        0,
                                                        0,
                                                        $newTagToolTipImageWidth,
                                                        $newTagToolTipImageHeight,
                                                        $imageWidth,
                                                        $imageHeight)
                                )
                                ...
                                if (!imagejpeg($newImage,
                                               "images/projects/{$projectMetadata['project_id']}/tooltips/$newTagToolTipImage",
                                               75)
                                )
                                {
                                ...
                                imagedestroy($originalImage);
                                imagedestroy($newImage);
                                if (!chmod("images/projects/{$projectMetadata['project_id']}/tooltips/$newTagToolTipImage",
                                           0775)
                                )
                                {
                                    $processingFailure[] = 'newTagToolTipImage';
                        ...
                        if ($newParentId != $oldParentId && !$processingFailure)
                        {
                            ...
                            $newParentContainerOrderResult =
                                run_prepared_query($DBH,
                                                   ...
                                                   $newParentContainerOrderParams);
                            $newParentContainerOrder = $newParentContainerOrderResult->fetchAll(PDO::FETCH_ASSOC);
                            $numberOfNewSiblings = count($newParentContainerOrder);
                            ...
                            $oldParentContainerOrderResult =
                                run_prepared_query($DBH,
                                                   ...
                                                   $oldParentContainerOrderParams);
                            $oldParentContainerOrder = $oldParentContainerOrderResult->fetchAll(PDO::FETCH_ASSOC);
                            $numberOfOldSiblings = count($oldParentContainerOrder);

                            for ($i = 0; $i < $numberOfOldSiblings; $i++)
                            {
                            ...
                            if (!$numberOfNewSiblings || $newParentContainerOrderResult)
                            {
                                //////////////////////////////////////////////////////////////////////////////
                                // Update the database. Old Container
                            ...
                            if (!$processingFailure)
                            {
                                ...
                                if ($groupOrderChanged)
                                {
                                    foreach ($parentContainerOrder as &$individualTag)
                                    {
                                        ...
                                        {
                                            if ($individualTag ['order_number'] > $oldTagOrder &&
                                                $individualTag ['order_number'] <= $newTagOrder
                                            )
                                            {
                                                $individualTag['order_number']--;
                                        ...
                                        elseif ($groupOrderChanged == 'down' &&
                                                $individualTag ['child_id'] != $updateTagId
                                        )
                                        {
                                            if ($individualTag ['order_number'] < $oldTagOrder &&
                                                $individualTag ['order_number'] >= $newTagOrder
                ...
                {
                    if ($projectEditSubAction == 'createNewTag' && !$invalidRequiredField)
                    {
                        ...
                        $tagAlreadyPresentResult =
                            run_prepared_query($DBH,
                                               $tagAlreadyPresentQuery,
                                               $tagAlreadyPresentParams);
                        ...
                        if (!$existingTagId)
                        {
                            ...
                            if (!$newNoTooltipImageStatus && !empty($newTagToolTipImage))
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    imagedestroy($originalImage);
                                    imagedestroy($newImage);
                                    if (!chmod("images/projects/{$projectMetadata['project_id']}/tooltips
                            /$newTagToolTipImage",
                                               0775)
                                    )
                                    {
...
EOL;
    ...
    {
        if ($importStatus == 12 || $importStatus == 13)
        { // END if ($importStatus == 11)
            $buildFormHTML .= '
            <input type="radio" name="projectPropertyToUpdate" value="tasks" id="editProjectTasks" />
            <label for="editProjectTasks" class="clickableButton editProjectAction">Tasks</label>
                        ...
                        for ($i = 0; $i < $numberOfTasks; $i++)
                        {
                            ...
                            if ($projectTasks [$i]['task_id'] == $taskId)
                            {
                                ...
                                $currentTaskDescription =
                                    restoreSafeHTMLTags(htmlspecialchars($currentTaskDescription));
...
EOL;
                    ...
                    {
                        if ($projectEditSubAction == 'createNewTask' || $importStatus != 11)
                        {
        ...
        case 'groups':
            ...
            if (!$projectEditSubAction && $importStatus != 12)
            {
                ...
                foreach ($projectGroups as $individualGroup)
                {
                    ...
                    $individualGroupDescription =
                        restoreSafeHTMLTags(htmlspecialchars($individualGroup['description']));
                    $isEnabled = $individualGroup['is_enabled'];
                    if ($isEnabled == 1)
                    {
                    ...
                    if ($groupHasContents) {
                        $deleteGroupButtonHTML = <<<HTML
                            <input class="clickableButton disabledClickableButton" 
                                ...
                                value="Delete Group"  
                                title="This group cannot be deleted at this time as it contains other groups or 
                                tags."                                                           
                                type="button"
                                disabled>
HTML;
                        ...
                        $deleteGroupButtonHTML = <<<HTML
                            <input class="clickableButton" 
                                id="deleteGroupButton" 
                                value="Delete Group"  
                                title="Deleting a group will remove it permanently from your 
                                    questions set. This can not be undone."
                                type="button">
HTML;
...
EOL;
                    ...
                    foreach ($groupOrderInParentTask as &$parentContainer)
                    {
                        ...
                        for ($i = 0; $i < count($parentContainer); $i++)
                        {
                            ...
                            $javascriptGroupOrderInParentTask[$parentId]['tableHTML'] =
                                $javascriptGroupOrderInParentTask [$parentId]['tableHTML'] . " title=\"{$parentContainer[$i]["display_text"]}\"><td>$ordinalOrderInTask</td><td>{$parentContainer[$i]["name"]
                                    }</td></tr>\n\r";
                        }
                    ...
                    if (count($groupIdList) > 0)
                    {
                        ...
                        foreach ($groupOrderInParentGroup as &$parentContainer)
                        {
                            ...
                            for ($i = 0; $i < count($parentContainer); $i++)
                            {
                                ...
                                $javascriptGroupOrderInParentGroup[$parentId]['tableHTML'] =
                                    $javascriptGroupOrderInParentGroup [$parentId]['tableHTML'] . " title=\"{$parentContainer[$i]["display_text"]}\"><td>$ordinalOrderInGroup</td><td>{$parentContainer[$i]["name"]
                                        }</td></tr>\n\r";
                            }
                            if (!$containerContainsGroup)
                            {
                        ...
                        {
                            if (!array_key_exists($singleGroupId,
                                                  $javascriptGroupOrderInParentGroup)
                            )
                            {
                                $javascriptGroupOrderInParentGroup[$singleGroupId]['tableHTML'] = '';
                                $javascriptGroupOrderInParentGroup[$singleGroupId]['newOrderSelectHTML'] =
                    ...
                    $actionControlsHTML = <<<EOL
                            ...
                            <form method="post" id="editGroupDetailsForm" autocomplete="off">
                                ...
                                <div class="formFieldRow">
                                    <label title="Deleting a group will remove it permanently from your 
                                    questions set. This can not be undone. If this option is unavailable then 
                                    the group either contains other groups or tags which must be moved elsewhere or 
                                    deleted first.">Delete Group:</label>
...
EOL;
                        ...
                        {
                            if (!array_key_exists($taskId,
                                                  $javascriptGroupOrderInParentTask)
                            )
                            {
                                $javascriptGroupOrderInParentTask[$taskId]['tableHTML'] = '';
                                $javascriptGroupOrderInParentTask[$taskId]['newOrderSelectHTML'] =
                                    "<option value=\"1\" selected>1st</option>\n\r";
                            }
                        }
                        $javascriptGroupOrderInParentTask = json_encode($javascriptGroupOrderInParentTask);
                        ...
                        if (count($groupIdList) > 0)
                        {
                            ...
                            foreach ($groupIdList as $groupId)
                            {
                                ...
                                )
                                {
                                    ...
                                    $javascriptGroupOrderInParentGroup[$groupId]['newOrderSelectHTML'] =
                                        "<option value=\"1\" selected>1st</option>\n\r";
                                }
                            }
                            $javascriptGroupOrderInParentGroup = json_encode($javascriptGroupOrderInParentGroup);
...
EOL;
                    ...
                    foreach ($tagOrderInParentGroup as &$parentContainer)
                    {
                        ...
                        for ($i = 0; $i < count($parentContainer); $i++)
                        {
                            ...
                            $javascriptTagOrderInParentGroup[$parentId]['tableHTML'] =
                                $javascriptTagOrderInParentGroup [$parentId]['tableHTML'] . " title=\"{$parentContainer[$i]["display_text"]}\"><td>$ordinalOrderInGroup</td><td>{$parentContainer[$i]["name"]
                                    }</td></tr>\n\r";
                        }
                    ...
                    $actionControlsHTML = <<<EOL
                            ...
                            <form enctype="multipart/form-data" method="post" id="editTagDetailsForm" autocomplete="off">
                                ...
                                <div class="formFieldRow">
                                    ...
                                    question set. This can not be undone.">Delete Tag:</label>
                                    <input class="clickableButton" 
                                        id="deleteTagButton" 
                                        value="Delete Tag"  
                                        title="Deleting a tag will remove it permanently from your question set. This 
                                        can not be undone."
                                        type="button">
                                </div>
...
EOL;
                        ...
                        {
                            if (!array_key_exists($groupId,
                                                  $javascriptTagOrderInParentGroup)
                            )
                            {
                                $javascriptTagOrderInParentGroup[$groupId]['tableHTML'] = '';
                                $javascriptTagOrderInParentGroup[$groupId]['newOrderSelectHTML'] =
                                    "<option value=\"1\" selected>1st</option>\n\r";
                            }
                        }
                        $javascriptTagOrderInParentGroup = json_encode($javascriptTagOrderInParentGroup);
