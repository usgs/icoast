...
$logFilename = 'importLog.html';

function abort_import($DBH = null, $failureReason = null, $importCollectionId = null) {
  global $logFilename;
  //file_put_contents($logFilename, date('H:i:s') . ": " . "ABORT - $importCollectionId: $failureReason<br>", FILE_APPEND);
...
//file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . array_dump(INPUT_POST));
...
//file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "$importCollectionId, $importKey, $userId, $checkCode<br>", FILE_APPEND);

$userMetadata = retrieve_entity_metadata($DBH, $userId, 'user');
//file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . array_dump($userMetadata, FILE_APPEND));
...
//file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "Auth<br>", FILE_APPEND);


//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Check update permission and project state validity
if (isset($importCollectionId)) {
  //file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "$importCollectionId<br>", FILE_APPEND);
    $importCollectionMetadata = retrieve_entity_metadata($DBH, $importCollectionId, 'importCollection');
    if ($importCollectionMetadata) {
      //file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "MetaData $importCollectionMetadata<br>", FILE_APPEND);
        if (isset($importKey) &&
                (!empty($importKey)) &&
                $importCollectionMetadata['import_key'] == $importKey) {
          //file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "Import Key OK<br>", FILE_APPEND);
            if (!empty($importCollectionMetadata['session_id']) && !empty($importCollectionMetadata['import_start_timestamp'])) {
              ...
              //file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "$elapsedTime<br>", FILE_APPEND);
                if ($elapsedTime < 10) {
                  //file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . "elapsedTime < 10<br>", FILE_APPEND);
...
//    unlink($logFilename);
//}
//file_put_contents($logFilename, date('H:i:s') . ": " . date('H:i:s') . ": " . 'Start'. "\r\n", FILE_APPEND);
...
//file_put_contents($logFilename, date('H:i:s') . ": " . 'Existing Folder check complete'. "\r\n", FILE_APPEND);

do {
    $failedImagesToRetry = array();
    $retryCount++;
//     file_put_contents($logFilename, date('H:i:s') . ": " . 'Loop 1'. "\r\n", FILE_APPEND);
    if ($retryCount == 3) {
//         file_put_contents($logFilename, date('H:i:s') . ": " . 'retrying = 3'. "\r\n", FILE_APPEND);
        $setProcessingFlagQuery = '
            UPDATE import_collections
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Image {$image['filename']} : $initalTimeStamp\r\n", FILE_APPEND);
...
//         file_put_contents($logFilename, date('H:i:s') . ": " . "Processing {$image['import_image_id']}". "\r\n", FILE_APPEND);
        userAbortCheck($importCollectionMetadata, $processedImageCount);
        ...
        if ($serverStringStartPosition !== false) {
            ...
            $localPath = TRUE;
//            file_put_contents($logFilename, date('H:i:s') . ": " . "Using local file path: $importImagePath\r\n", FILE_APPEND);
        } else {
            $importImagePath = $image['imageURL'];
//            file_put_contents($logFilename, date('H:i:s') . ": " . "Using URL file path: $importImagePath\r\n", FILE_APPEND);
        }
// Remove after development
        ...
        if (!@$originalImage = imagecreatefromjpeg($importImagePath)) {
            if ($localPath) {
                $importImagePath = $image['imageURL'];
//                file_put_contents($logFilename, date('H:i:s') . ": " . "Local path failed switching to URL path: $importImagePath\r\n", FILE_APPEND);
                if (!@$originalImage = imagecreatefromjpeg($importImagePath)) {
//                    file_put_contents($logFilename, date('H:i:s') . ": " . "URL not found. Adding to retry.\r\n", FILE_APPEND);
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Images Downloaded: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);
        ...
        // file_put_contents($logFilename, date('H:i:s') . ": " . 'Image Creation COmplete'. "\r\n", FILE_APPEND);
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Images Created: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);

...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Feature Search: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Major City Search: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);

        // Add the nearest city to the $csvRow array.
        if ($city) {
            // file_put_contents($logFilename, date('H:i:s') . ": " . "MaX POP", FILE_APPEND);
            foreach ($city as $param => $value) {
                // file_put_contents($logFilename, date('H:i:s') . ": " . $param . "=>" . $value . "\r\n", FILE_APPEND);
            }
            ...
            if ($county) {
                // file_put_contents($logFilename, date('H:i:s') . ": " . "County = $county\r\n", FILE_APPEND);
                $image['county'] = $county;
            }
        } else {
            // file_put_contents($logFilename, date('H:i:s') . ": " . "Unlimited", FILE_APPEND);
...
//            file_put_contents($logFilename, date('H:i:s') . ": " . "Minor City Search: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);
            // Add the nearest city to the $csvRow array.
            if ($city) {
                foreach ($city as $param => $value) {
                    // file_put_contents($logFilename, date('H:i:s') . ": " . $param . "=>" . $value . "\r\n", FILE_APPEND);
        ...
        if (!$city) {
//            file_put_contents($logFilename, date('H:i:s') . ": " . 'No City' . "\r\n", FILE_APPEND);
        }
        // file_put_contents($logFilename, date('H:i:s') . ": " . 'GeoCoding complete'. "\r\n", FILE_APPEND);
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Geo Reference Total: $actionElapsedTime : $totalElapsedTime\r\n", FILE_APPEND);

...
//        }
        ...
        if ($insertId) {
//             file_put_contents($logFilename, date('H:i:s') . ": " . 'IMage DB insert success'. "\r\n", FILE_APPEND);
            $processedImageCount++;
...
//             file_put_contents($logFilename, date('H:i:s') . ": " . 'IMage DB insert failed'. "\r\n", FILE_APPEND);
            unlink("../images/temporaryImportFolder/{$importCollectionMetadata['import_collection_id']}/main/{$image['filename']}");
...
//        file_put_contents($logFilename, date('H:i:s') . ": " . "Row Inserted: $actionElapsedTime : $totalElapsedTime\r\n\n", FILE_APPEND);
    }
    $imageArray = $failedImagesToRetry;
} while (!empty($failedImagesToRetry) && $retryCount <= 2);
// file_put_contents($logFilename, date('H:i:s') . ": " . 'Looping complete'. "\r\n", FILE_APPEND);
...
// file_put_contents($logFilename, date('H:i:s') . ": " . 'Done'. "\r\n", FILE_APPEND);
